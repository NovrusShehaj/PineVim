...
The bundled extension installs the PineVim frame inside the Pi pane through Pi's public extension API: header, status deck, chip band above the composer, lifecycle working indicator, one run-ledger line per user request, `/ide` and `/pinevim` argument completion, and non-persistent themes (`pinevim-dark`, `pinevim-light`, `pinevim-mono`, `pinevim-neon`, `pinevim-cyberpunk`, `pinevim-forest`, `pinevim-snow`). `pinevim-cyberpunk` adapts Cyberpunk Neon by Theo (theo-grivel), CC-BY-SA-4.0, from https://github.com/Roboron3042/Cyberpunk-Neon. `ui.theme: "auto"` applies `pinevim-dark` or `pinevim-light` only when Pi is still on its built-in `dark` or `light` theme. Any other theme, including one you picked, is left alone. Applying a theme passes a theme object, so PineVim does not write Pi's `settings.json`. Theme files are copied into Pi's user themes directory so `/settings` can resolve them later; that copy is outside PineVim's private state directory and is skipped when it fails. The frame now has a clearer information hierarchy: the header is the identity rail (PineVIM mark, workspace, session, and the current view/focus); the band above the composer is the live activity rail (state, tool, run elapsed time, queue, and attention); and the deck is a compact `ENV` row for context, model, thinking level, branch, and the key hint. With 25 or more usable pane rows, the 11-line launch splash is eligible. At 24 rows or fewer it yields immediately to a four-line compact identity mark, so 60×16 and 80×24 launches keep the transcript and composer visible. A first-run welcome entry points to `/pinevim help`, `/pinevim review`, and `/pinevim skills`, while each settled request gets a quiet, expandable run ledger. The ASCII pine remains the brand mark — canopy in the theme's accent, trunk in a shaded tone of the same hue (darker on dark themes, lifted on light ones; per-theme overrides live in `src/piui/logo.ts`, preview with `node scripts/pine-preview.mjs [#rrggbb] [--light]`). The terminal title reads `pinevim`. Pi keeps editing, Markdown, model dialogs, tool execution, and keybindings. There is no custom editor component.

The tmux control plane mirrors the same state language with terminal-safe colors: the bottom strip emphasizes the view and agent lifecycle, `F12 s` opens a scannable workspace-status panel, `F12 ?` opens a sectioned key guide, and `F12 m` exposes the same actions as a menu. The `ui.glyphs: "ascii"` setting swaps the frame and panels to terminal-safe fallbacks without changing their layout.
...
Neovim starts normally in the workspace with `PINEVIM=1`. An intentional `NVIM_APPNAME` is preserved. Inherited remote-editor markers are removed for the editor child. After your config loads, PineVim applies the same theme JSON the Pi pane is using: transparent background so both panes share the terminal, syntax and diagnostics in the pine/teal/sand palette, a single status line, and flat separators instead of powerline arrows. `ui.theme: "auto"` follows Neovim's `background` (`pinevim-dark` or `pinevim-light`); an explicit `pinevim-*` name uses that file. Your Neovim config still owns LSP, completion, diagnostics, formatting, git UI, tests, and debugging. PineVim does not edit that config, install a plugin, or use Neovim RPC. Prefix keys default to F12 so Pi's Ctrl+L and Neovim's Ctrl+H/J/K/L stay with those programs. Ctrl+C goes to the focused child.
...
Header snapshots are never rewritten by tests. Tests and the updater share one manifest; the updater rejects missing, duplicate, or unmanifested header fixtures. After an intentional header layout change, run `npm run snapshots:update`, review the fixture diff, and then run the normal checks.
